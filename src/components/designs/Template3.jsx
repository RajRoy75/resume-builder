...
// If you are using react-router-dom, keep these imports
...
// Assuming Template ID might be passed via props or context if not using react-router
const Template3 = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    mobile: "02800200",
    home: "02800200", // Assuming 'Home' is another phone number
  ...
  const [education, setEducation] = useState([
    ...
    {
       degree: "BECHELOR OF ART DIRECTOR", // Note: Typo in original image is "BECHELOR"
       university: "UNIVERSITY OF LOREM",
       dates: "2007 - 2009",
       description: "Porttitor amet massa Done cporttitor dolor et nisl molestie tum feliscon lore. Ipsum dolor stringilla lorem lorem ipsum, olicitudin est dolor time. Done cporttitor.",
     },
     // The image shows two 'BECHELOR OF ART DIRECTOR' entries with the same dates. Adjust if needed.
     {
  ...
  const toggleEditable = () => {
    setIsEdit(!isEdit);
    // This logic to make inputs/textareas read-only is handled by the `readOnly` prop now
  };
  ...
  const saveFormData = async () => {
    ...
    const _doc = {
      _id: loadedTemplateId, // Or generate a new ID if it's a new save
      resume_id,
      formData,
      education,
      experiences,
      skills,
      references, // Add references to save data
      timeStamp,
      imageUrl, // Save the preview image URL
      // userProfilePic: null, // No profile pic for this template
    };
    ...
    setDoc(doc(db, "users", user.uid, "resumes", resume_id), _doc)
      .then(() => {
        ...
        if (refetch_resumeData) refetch_resumeData(); // Refetch data if using react-query
      })
      .catch((err) => {
  ...
  const getImage = async () => {
    ...
    try {
      // Using toJpeg for smaller file size, quality 0.95
  ...
  // PDF Generation
    const generatePDF = async () => {
      const element = resumeRef.current;
      if (!element) {
          toast.error("Unable to capture resume content.");
          return;
      }

      try {
          // Ensure the element dimensions are captured correctly before generating the image
          const dataUrl = await htmlToImage.toPng(element);

          const pdf = new jsPDF('p', 'mm', 'a4'); // A4 dimensions: 210 x 297 mm
          ...
          img.onload = () => {
              ...
              // Calculate the dimensions to fit the image onto the A4 page
              let newImgWidth = pdfWidth;
              let newImgHeight = newImgWidth / aspectRatio;

              // If the calculated height exceeds the page height, scale by height instead
          ...
          img.onerror = () => {
             toast.error("Failed to load captured image for PDF generation.");
          }
  ...
  // --- RENDER ---
  return (
    <div className="flex flex-col items-center justify-start gap-4 p-4 lg:p-8">
      {/* Controls */}
      ...
      {/* Using max-w-4xl for a reasonable max width */}
